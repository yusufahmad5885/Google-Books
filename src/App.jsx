...
function App() {
  ...
  const handleSubmit = () => {
    ...
    if (maxResults > 40 || maxResults < 1){
      ...
      axios
        .get(
        `https://www.googleapis.com/books/v1/volumes?q=${query}&maxResults=${maxResults}&startIndex=${startIndex}`).then(res => {

        if (startIndex >= res.data.totalItems || startIndex < 1){
          toast.error(`max data must be between 1 and ${res.data.totalItems}`);
        }
        else{
